...
extern unsigned char Pic16F_firmware_data_15399[];
extern int pic_fw_ver_count_15399;
...
static int bq27541_soc_calibrate(struct opchg_bms_charger *di, int soc)
{
	...
	//stap5: debug_log
	counter_debug++;
	if((counter_debug >= 9)||(soc_temp != di->soc_pre))
	{	
...
static void fastcg_work_func(struct work_struct *work)
{
	...
	//pr_err("%s is start\n",__func__);
	if(is_project(OPPO_15011) || is_project(OPPO_15399) || is_project(OPPO_15022))
	{
	...
	else if(fw_ver_info)
	{
		...
		{
			if((!pic_have_updated) && (Pic16F_firmware_data_15399[pic_fw_ver_count_15399 - 4] > data)){
				ret_info = 0x2;
				pic_need_to_up_fw = 1;	//need to update fw
			}else{
				ret_info = 0x1;
		...
		{
			pr_err("local_fw:0x%x,need_to_up_fw:%d\n",Pic16F_firmware_data_15399[pic_fw_ver_count_15399 - 4],pic_need_to_up_fw);
		}
		else if(is_project(OPPO_15022))
		{
...
out:
	...
	if(is_project(OPPO_14005)||is_project(OPPO_14023)||is_project(OPPO_15011)||is_project(OPPO_15399) || is_project(OPPO_15022))
	{
		...
		if(is_project(OPPO_15011) || is_project(OPPO_15399) || is_project(OPPO_15022)){ //add for int trigger mistakenly
			rc =opchg_set_clock_active(bq27541_di);
